...
| 亮点 | 说明 |
|---|---|
| **实时工具调用** | Function Calling 按需调用外部能力（天气查询等），让大模型从"只聊天"变成"能办事" |
| **成本优化** | 语义缓存拦截重复提问、历史消息摘要压缩上下文、Token 估算前置拦截超限请求——把单次对话成本压到原来的 20% 以内 |
| **高可用（重试/熔断/降级/兜底）** | 瞬时抖动用重试抗<br>熔断器硬软错误分类止损：配额耗尽单次即熔断，5xx 累计 3 次开闸<br>指数退避避免雪崩（hardMul 翻倍冷却）<br>全部熔断时关键词兜底保证"不黑屏"<br>四层递进，用户感知不到后端故障 |
| **可观测性** | Prometheus 指标（含熔断器状态）+ OpenTelemetry 分布式追踪 + Langfuse（LLM 专项）+ 审计日志——模型调用链路从请求入口到 token 消耗全可追溯 |
