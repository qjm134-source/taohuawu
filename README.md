...
| 亮点 | 说明 |
|---|---|
| **工具调用** | Function Calling 按需调用外部能力——天气查询、游戏规则查询，支撑景区智能导游 / 虚拟导览等业务场景，让大模型从"只聊天"变成"能办事" |
| **记忆管理** | 语义缓存拦截重复提问、历史消息摘要压缩上下文、Token 估算前置拦截超限请求——把单次对话成本压到原来的 20% 以内 |
| **可用性** | **重试** 抖动立即重试；<br>**降级、熔断** 主模型不可用时，降级到备用模型，自动熔断切换；<br>**兜底** 全部模型不可用时，兜底回复预定义说辞；|
| **可观测性** | Prometheus 指标 + OpenTelemetry 分布式追踪 + Langfuse（LLM 专项）+ 审计日志——模型调用链路从请求入口到 token 消耗全追溯 |
