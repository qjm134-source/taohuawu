...
江南水乡智能导游系统是一个**游戏化 + AI 对话**的实战项目。玩家通过浏览器进入一个江南水乡的 2D 场景，与 NPC 导游"小荷"进行实时对话，完成景区智能导游 / 虚拟导览服务。后端基于 WebSocket 提供长连接服务，并基于 Eino 框架构建多模型 LLM 路由系统，实现智能、稳定、低成本的对话体验。
...
| 亮点 | 说明 |
|---|---|
| **沉浸式对话** | **连贯性**：记忆管理模块，保留多轮对话的上下文语境；<br> **准确性**：游戏规则知识库检索、天气工具调用，支撑导游 / 导览场景，降低幻觉；<br> **个性化**：情绪检测与玩家画像，让回复兼具温度与个性化； |
| **成本优化** | **语义缓存**：实现重复、相似问题拦截，降低 Token 消耗及响应延迟；<br>**历史对话摘要**：避免窗口超限，注意力分散； |
| **可用性** | **重试**：瞬时抖动自动重试；<br>**降级 / 熔断**：主模型不可用时，降级至备用模型并触发熔断切换；<br>**兜底**：全部模型不可用时返回预置回复； |
| **可观测性** | **系统指标**：Prometheus 采集 QPS、延迟、成本、缓存命中率等运行健康度指标； <br> **全链路追踪**：OpenTelemetry + Langfuse（LLM 专项），可追溯每次 LLM 调用完整细节，包括输入 Prompt、输出、Token、耗时；<br>**审计日志**：完整留存请求与响应，支撑安全审计与问题回溯； |
...
```text
┌──────────────────────────────────────────────────────────────────────┐
│                上下文分层架构（Context Engineering）                  
├──────────────────────────────────────────────────────────────────────┤
│ 【Layer 0 系统层】固定注入                                        
│  ├─ 角色定义：NPC“小荷”身份、职责、回复风格                          
│  └─ 世界观约束：江南水乡背景设定                                     
├──────────────────────────────────────────────────────────────────────┤
│ 【Layer 1 状态层】固定注入                                           
│  ├─ 玩家信息（昵称）                                              
...
├──────────────────────────────────────────────────────────────────────┤
│ 【Layer 2 知识层】按需注入                               
│  ├─ 知识检索（LLM 自主决策调用，隐式意图路由）                        
│  │    ├─ 游戏规则 -> 规则库                                       
│  │    ├─ 游戏场景 -> 场景库                                       
│  │    └─ 常见问答 -> FAQ 库                                       
│  └─ 工具调用（实时天气查询）                                        
├──────────────────────────────────────────────────────────────────────┤
│ 【Layer 3 记忆层】优先级动态裁剪（Token 超阈值触发压缩）               
│  ├─ 短期记忆：最近 N 轮原始对话                                     
│  └─ 历史记忆：超 Token 阈值触发 LLM 增量摘要，替换为 [对话摘要]         
├──────────────────────────────────────────────────────────────────────┤
│ 【Layer 4 会话层】动态注入                                         
│  └─ 当前用户问题                             
└──────────────────────────────────────────────────────────────────────┘
...
5. **成本优化与 Memory 管理**
   ...
   **本项目选择 LLM 增量摘要的原因**：
   ...
   ```
6. **高可用设计**
   - 熔断器：硬错误（配额耗尽 / 鉴权失败）单次即熔断，软错误（5xx / 超时 / 429）累计达阈值开闸，半开探测恢复。
   - 多模型降级：主模型故障自动切换至备用模型，单点故障不影响服务。
   - FallbackAdapter：所有模型不可用时返回预设兜底回复，保证不黑屏。
7. **可观测性**
   - Prometheus 指标暴露 `/metrics`（含 Token、成本、熔断器状态、缓存命中率），已集成到 docker-compose。
   - OpenTelemetry 分布式追踪，支持 stdout（开发）和 OTLP（生产）。
   - 审计日志支持多租户查询。
   - 已集成 Langfuse，通过 OTLP 导出 LLM 调用链路（Token、成本、Prompt 全链路追溯），详见 [可观测性指南](./backend/docs/OBSERVABILITY.md)。
8. **前端游戏化交互**
   - Phaser 3 2D 场景 + 像素风。
