...
import (
	...
	"github.com/watertown/guide/internal/adapter/tools"
	"github.com/watertown/guide/internal/config"
	"github.com/watertown/guide/internal/core/cost"
	"github.com/watertown/guide/internal/core/emotion"
...
func (r *Runtime) HandleWelcome(ctx context.Context, sess *session.Session) (string, error) {
	// 总超时
	...
	cacheKey := "welcome_" + sess.PlayerID
	...
	if actual, loaded := r.inflightWelcome.LoadOrStore(sess.PlayerID, notifyChan); loaded {
		select {
		...
		r.inflightWelcome.Delete(sess.PlayerID)
		// 递归重试一次（带 guard 防止无限递归）
		return r.handleWelcomeInternal(ctx, sess, cacheKey)
	}
	// 确保完成后通知等待者
	defer func() {
		r.inflightWelcome.Delete(sess.PlayerID)
...
func (r *Runtime) handleWelcomeInternal(ctx context.Context, sess *session.Session, cacheKey string) (string, error) {
	startTime := time.Now()

	prompt := BuildWelcomePrompt(sess.Nickname)
	messages := r.buildWelcomeMessages(prompt)

	msg, usage, err := r.llm.chat(ctx, messages, defaultTemperature, welcomeMaxTokens)
	if err != nil {
		return r.handleWelcomeError(ctx, startTime, cacheKey, sess, err)
	}

	return r.processWelcomeResponse(ctx, startTime, cacheKey, sess, msg, usage)
}
...
func (r *Runtime) handleWelcomeError(ctx context.Context, startTime time.Time,
	cacheKey string, sess *session.Session, err error) (string, error) {
	...
	if r.config.FallbackResponse.Enabled && r.config.FallbackResponse.WelcomeMessage != "" {
		...
		sess.AddMessage("assistant", reply, "neutral", nil)
...
func (r *Runtime) processWelcomeResponse(ctx context.Context, startTime time.Time,
	cacheKey string, sess *session.Session, msg *eino_schema.Message, usage *llm.ChatUsage) (string, error) {
...
// HandleChat 处理聊天（非流式）
func (r *Runtime) HandleChat(ctx context.Context, sess *session.Session, message string) (string, string, *LLMStats, error) {

	ctx, cancel := utils.WithTimeoutFrom(ctx, r.config.Timeout)
	defer cancel()

	ctx = contextutil.WithSessionID(ctx, sess.ID)

	ctx, span := observability.StartSpanWithStartTime(ctx, "Agent.HandleChat",
		trace.WithAttributes(
			observability.SessionID.String(sess.ID),
			observability.UserID.String(sess.PlayerID),
			observability.LangfuseTagTenant.String(sess.TenantID),
...
func (r *Runtime) callLLMAndProcess(ctx context.Context, span trace.Span,
	sess *session.Session, message, emotionStr, cacheKey string) (string, string, *LLMStats, error) {

	messages := r.buildContextMessages(sess, message, emotionStr)
	...
	return r.processLLMResponse(span, sess, message, emotionStr, cacheKey, msg, usage)
}

func (r *Runtime) processLLMResponse(span trace.Span,
	sess *session.Session, message, emotionStr, cacheKey string, msg *eino_schema.Message, usage *llm.ChatUsage) (string, string, *LLMStats, error) {
	...
	sess.AddMessage("user", message, emotionStr, nil)
	if reply != "" {
		sess.AddMessage("assistant", reply, emotionStr, nil)
	}
...
func (r *Runtime) HandleChatStream(ctx context.Context, sess *session.Session, message string) (<-chan *llm.StreamEvent, <-chan *LLMStats, error) {
	startTime := time.Now()
...
func (r *Runtime) finalizeStream(ctx context.Context, span, llmSpan trace.Span, eventChan chan<- *llm.StreamEvent,
	...
	sess *session.Session, message, emotionStr, cacheKey string) {
...
func (r *Runtime) updateSession(llmCtx context.Context, sess *session.Session, message, emotionStr, reply string) {
...
func (r *Runtime) buildContextMessages(sess *session.Session, currentMessage string, emotion string) []*eino_schema.Message {
	allMessages := sess.GetMessages(0)
...
func (r *Runtime) estimateMessagesTokens(messages []session.Message, currentMessage string) int {
...
func (r *Runtime) buildFullHistoryMessages(messages []session.Message, currentMessage, emotion string) []*eino_schema.Message {
...
func (r *Runtime) buildSummarizedMessages(messages []session.Message, currentMessage, emotion string) []*eino_schema.Message {
...
func (r *Runtime) buildSummaryMessage(oldMessages []session.Message) []*eino_schema.Message {
...
func (r *Runtime) summarizeMessages(ctx context.Context, oldMessages []session.Message) string {
	summarizer := r.optimizer.GetSummarizer()
	if summarizer == nil {
		return ""
	}
...
func messagesToText(msgs []session.Message) string {
...
// GetSession 获取会话
func (r *Runtime) GetSession(playerID, tenantID string) *session.Session {
	return r.sessionManager.GetOrCreate(playerID, tenantID)
}

// MarkVisited 标记已访问
func (r *Runtime) MarkVisited(sessionID string) {
	if sess, ok := r.sessionManager.Get(sessionID); ok {
		sess.MarkVisited()
	}
}
