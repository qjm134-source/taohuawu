...
type Optimizer struct {
	...
	summary      *Summary
	embeddingAPI cache.EmbeddingAPI
...
// NewOptimizer 创建优化器
func NewOptimizer(cacheTTL time.Duration, maxMessages, tokenLimit int, embeddingAPI cache.EmbeddingAPI, summarizer Summarizer, logger logging.Logger) *Optimizer {
	...
	return &Optimizer{
		cache:        cache.NewLayeredCache(cacheConfig, embeddingAPI, logger),
