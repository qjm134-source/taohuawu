...
import (
	...
	"github.com/watertown/guide/internal/adapter/cache"
	...
	"github.com/watertown/guide/internal/adapter/weather"
	"github.com/watertown/guide/internal/config"
	"github.com/watertown/guide/internal/core/agent"
	corecontext "github.com/watertown/guide/internal/core/context"
	...
	"github.com/watertown/guide/internal/handler/ws"
...
func (s *Server) SetWebSocketHandler(handler *ws.WebSocketHandler) {
	s.wsHandler = handler
}
...
func (s *Server) initWebSocketHub() *ws.Hub {
...
func (s *Server) initEmbeddingAPI() cache.EmbeddingAPI {
	if !s.config.Cost.Embedding.Enabled {
		return nil
	}

	if s.config.Cost.Embedding.Type == "local" {
		return cache.NewLocalEmbeddingClientWithConfig(cache.LocalEmbeddingConfig{
	...
	return cache.NewOpenAIEmbeddingClient(
...
func (s *Server) initSummarizer(adapter llm.Adapter) cost.Summarizer {
	...
	return corecontext.NewLLMSummarizer(adapter, summaryModel, summaryTimeout, s.logger)
}

func (s *Server) initOptimizer(embedding cache.EmbeddingAPI, summarizer cost.Summarizer) *cost.Optimizer {
...
func (s *Server) initWebSocketHandler() *ws.WebSocketHandler {
	return ws.NewWebSocketHandler(
		s.agentHub,
		s.sessionManager,
		s.agentRuntime,
		repository.NewPlayerRepository(s.db),
		repository.NewConversationRepository(s.db),
		s.auditRepo,
		s.logger,
	)
}
